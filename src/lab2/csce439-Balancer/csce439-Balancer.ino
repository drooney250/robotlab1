...
void sendDebugData(){
 ...
 Serial.print("Line Sensor 5:"); Serial.println(sensorValues[4]);
  Serial.print("Range Finder:"); Serial.println(rangeFinderReading);
...
void sendDataToROS(){
 ...
 if((currentMS - lastMS) < 100) return;
  ...
  sendSignedInt32MSBAndUpdateChecksum(rangeFinderReading,&checksum);
...
void loop()
{
  ...
  rangeFinderReading = analogRead(rangeFinderPin);
  sendDataToROS();
  processDataFromROS();
